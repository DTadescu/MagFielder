...
class ModelMediator(sPref: SharedPreferences) {
    ...
    private var resulListeners: ArrayList<(FieldResult) -> Unit> =
        arrayListOf(fun(_: FieldResult) {})
    ...
    fun doWork(lat: Double, lon: Double, alt: Double, _date: String) {
        try {
            ...
            Log.d(
                "CALC",
                "Geoparams is done ${gParams.radius} ${gParams.theta} ${gParams.sd} ${gParams.cd}"
            )
